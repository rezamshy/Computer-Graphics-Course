...
function render() {
    ...
    world.list.push(new Tetrahedron(new Vector(0, 0, 0), 4, new LambertianMaterial(new Solid_Color(0.6 , 0.6 , 1))));


    let r = 4;

    for(i = 0; i < 20; i++){
        let theta = i * (2 * Math.PI / 20);
        let z = - r * Math.sin(theta);
        let x = r * Math.cos(theta);
        let y = 0.4;
        world.list.push(new Sphere(new Vector(x, y, z), y, new LambertianMaterial(new Solid_Color(Math.random(), Math.random(), Math.random()))));
    }

    let lookFrom = new Vector(-3, 3.5, 10);
    let lookAt = new Vector(0, 1, 0);
