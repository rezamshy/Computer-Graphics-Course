...
function main() {
  ...
  canvas.onmousedown = function(ev) { isMouseDown=true };
  canvas.onmouseup   = function(ev) { isMouseDown=false } ;
  canvas.onmousemove =  function(ev) { if(isMouseDown) moved(ev, groundPlane, cube); };
...
function moved (ev, groundPlane, cube){
  ...
  if (Math.abs(x-lastx)>Math.abs(y-lasty)){
    ...
    if(y-lasty>0)
      ...
      draw(1.1, groundPlane, cube,1);
    else
      
      draw(0.9, groundPlane, cube,1);
  }
  lastx=x;
  lasty=y;
